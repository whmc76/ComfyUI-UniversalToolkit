    ...
    data = {
        ...
        "presets": [
            ...
            },
            {
                ...
                "brief": "Transform the product into a professional photography scene based on $user_prompt$, ensuring the product's shape, texture, colors, and surface details remain completely unchanged. The scene description from $user_prompt$ has the highest priority, and the environment must accurately reflect this specified context (e.g., shopping mall window display, cozy café table, outdoor park bench) rather than defaulting to a studio backdrop. Apply a commercial-grade transformation with the following principles: 1. **Scene-Driven Environment Priority**: Strictly follow the environment specified in $user_prompt$. Only use a clean studio backdrop if explicitly requested. For other contexts, create a professional setting that matches the scene while maintaining commercial photography quality and visual clarity. 2. **Professional Lighting & Shadow Control**: Use lighting setups that match the requested environment (e.g., mall ambient lighting with soft reflections, natural sunlight for outdoor, warm interior lighting for lifestyle). Adjust shadows and highlights to enhance the product while ensuring the lighting feels authentic to the scene. 3. **Composition & Focus Excellence**: Frame the product using professional photography principles (e.g., rule of thirds, depth of field, natural foreground/background integration), keeping the product as the clear focal point. 4. **High-Quality Standards**: Ensure sharp focus, correct exposure, and professional color grading that enhances the product's design and integrates seamlessly with the scene's tone. 5. **Seamless Context Integration**: The product must appear naturally placed in the requested scene (e.g., within a shop display or lifestyle surface), with consistent reflections, shadows, and scale. 6. **Brand-Ready Presentation**: Final output must meet e-commerce and marketing standards, suitable for online catalogs, ads, or promotional images. Describe the exact scene setup, including camera angle, background elements, lighting style, and composition, ensuring it matches $user_prompt$ and avoids default studio interpretations unless explicitly mentioned.",
            },
            {
